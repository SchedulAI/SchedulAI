...
---
## Descrição ##
ScheduleAI é uma plataforma inteligende de agendamento desenvolvida para simplificar o processo de organizar reuniões. O aplicativo utiliza inteligência artifical para ajudar os usuários a encontrar os melhores horários, eliminando a necessidade de longas trocas de mensagens.

## Principais funcionalidades ##
- **Agendamento com IA:** O anfitrião do evento pode inserir a data e horário proposto do mesmo, e o ScheduleAI, alimentado por um modelo de linguagem avançado (LLM), convida todas as pessoas que o anfitrião indicar e repassa essas informações para eles, verificando a disponibilidade de cada um para então, retornar ao anfitrião com os possíveis ou não horários de agendamento para aquela reunião.
- **Interação com os usuários:** Após o anfitrião agendar um evento, ele recebe um link exclusivo para compartilhar com os convidados, e caso um convidado já possua conta na plataforma, o convite irá aparecer diretamente no seu dashboard. Os convidados podem criar uma conta atráves do link recebido e informar a disponibilidade possível dentro da disponibilidade do anfitrião.
...
- **Interface Amigável:** Desenvolvido com frontend em React e backend em TypeScript, o app oferece uma experiência de usuário simples e intuitiva.
...
3. Configure as variáveis de dependência a partir do .env_example.
4. Utilize o comando `npm run build` para construir a aplicação
5. Utilize o comando `npm run preview` para iniciar um servidor com os arquivos construidos.
...
8. Configure as variáveis de dependência a partir do .env_example.
...
11. Utilize o comando `npm start` para iniciar um servidor com os arquivos construidos.
...
3. Configure as variáveis de dependência a partir do .env_example.
4. Utilize o comando `npm run build` para construir a aplicação
5. Copie os arquivos da pasta **dist** para a pasta onde você serve os arquivos estáticos do nginx.
6. Altere o nginx-example.conf para condizer com sua maquina e copie para a pasta de configuração do nginx.
...
9. Configure as variáveis de dependência a partir do .env_example.
